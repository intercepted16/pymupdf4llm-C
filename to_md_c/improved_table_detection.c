...
static float analyze_table_content(fz_context *ctx, fz_stext_page *textpage, fz_rect table_bbox) {
    int numeric_count = 0;
    int total_count = 0;
...
static int detect_grid_structure(fz_rect *blocks, int block_count, 
                                float *h_lines, int *h_count, 
...
static table_features_t analyze_table_structure(fz_rect *blocks, int block_count, 
                                               fz_context *ctx, fz_stext_page *textpage) {
    ...
    if (textpage && block_count > 0) {
        ...
        features.content_score = analyze_table_content(ctx, textpage, bbox);
    }
...
static float is_likely_table(fz_rect *blocks, int block_count, 
                            fz_context *ctx, fz_stext_page *textpage) {
    if (block_count < 4) return 0.0f;
    
    table_features_t features = analyze_table_structure(blocks, block_count, ctx, textpage);
    ...
    if (((features.row_count >= 2 && features.col_count >= 1) || (features.row_count >= 1 && features.col_count >= 2)) &&
...
static int find_table_candidates(fz_rect *blocks, int block_count, 
                                table_candidate_t *candidates, int max_candidates,
                                fz_context *ctx, fz_stext_page *textpage) {
    ...
    for (int i = 0; i < block_count && candidate_count < max_candidates; i++) {
        ...
        for (int j = 0; j < nearby_count && region_count < 100; j++) {
            ...
            if (expanded_area < original_area * 4.0f) {  // Not too much expansion
                float aspect_ratio = (expanded.x1 - expanded.x0) / (expanded.y1 - expanded.y0);
                if (aspect_ratio > 0.2f && aspect_ratio < 10.0f) {  // Reasonable aspect
                    region = expanded;
        ...
        if (region_count >= 4) {
            ...
            if (score > 0.4f) {  // Threshold for table classification
...
fz_rect* find_all_tables_on_page(fz_context *ctx, fz_document *doc, int page_number, int* count) {
    *count = 0;
    ...
    fz_try(ctx) {
        ...
        if (!textpage) fz_rethrow(ctx);

        blocks = malloc(sizeof(fz_rect) * MAX_BLOCKS);
        if (!blocks) fz_rethrow(ctx);
        int block_count = 0;
        ...
        if (block_count < 4) {
            *count = 0;
            table_rects = NULL;
            fz_rethrow(ctx); // Jump to always block for cleanup
        }

        qsort(blocks, block_count, sizeof(fz_rect), compare_rects_by_top_left);

        candidates = malloc(sizeof(table_candidate_t) * MAX_CANDIDATES);
        if (!candidates) fz_rethrow(ctx);
        int candidate_count = find_table_candidates(blocks, block_count, candidates, MAX_CANDIDATES, ctx, textpage);

        if (candidate_count > 0) {
            table_rects = malloc(candidate_count * sizeof(fz_rect));
            if (!table_rects) fz_rethrow(ctx);

            int actual_count = 0;
            for (int i = 0; i < candidate_count; i++) {
                if (candidates[i].score > 0.4f) { // Confidence threshold
                    table_rects[actual_count++] = candidates[i].bbox;
                }
            }
            *count = actual_count;
...
int improved_page_has_table(const char *pdf_path, int page_number) {
    ...
    fz_try(ctx) {
        ...
        for (int p = start; p <= end; p++) {
            ...
            for (fz_stext_block *block = textpage->first_block;
                 block && surrounding_block_count < MAX_BLOCKS;
                 block = block->next) {
                if (block->type != FZ_STEXT_BLOCK_TEXT) continue;
            ...
            if (surrounding_block_count >= 4) {
                ...
                qsort(surrounding_blocks, surrounding_block_count, sizeof(fz_rect), compare_rects_by_top_left);
                
                // Find table candidates on surrounding pages
                table_candidate_t candidates[MAX_CANDIDATES];
                int candidate_count = find_table_candidates(surrounding_blocks, surrounding_block_count, 
                                                          candidates, MAX_CANDIDATES,
                                                          ctx, textpage);
                
                // Check if any surrounding page has a high-confidence table
                for (int i = 0; i < candidate_count; i++) {
                    if (candidates[i].score > 0.8f) {  // High confidence threshold
                        surrounding_table_found = 1;
                        break;
                    }
                }
                
                // Early termination if we found a high-confidence table in surrounding pages
        ...
        if (target_page) {
            ...
            if (target_textpage) {
                ...
                if (block_count >= 4) {
                    ...
                    table_candidate_t candidates[MAX_CANDIDATES];
                    int candidate_count = find_table_candidates(blocks, block_count, 
                                                              candidates, MAX_CANDIDATES,
                                                              ctx, target_textpage);
                    ...
                    for (int i = 0; i < candidate_count; i++) {
                        if (candidates[i].score > best_score) {
                            ...
                            if (best_score > 0.8f) {  // High confidence threshold
                                has_table = 1;
                                break;
                            }
                        }
    ...
    // This implements the requirement: "if im on page 28, if 29 or 27 have tables, then 28 now does"
