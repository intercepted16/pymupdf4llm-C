...
def to_markdown(
    ...
    *,
    ...
    profiler.start_timer('header_identification')
    if callable(hdr_info):
        ...
        get_header_id = hdr_info.get_header_id
    profiler.end_timer('header_identification')
    ...
    def write_text(
        ...
        Uses cached raw lines and filters for the region."""
        if clip is None:
            clip = parms.clip
        out_string = ""
        ...
        nlines = [
            ...
            if clip.intersects(l[0]) and not intersects_rects(l[0], parms.tab_rects.values())
        ]
    ...
    def output_tables(parms, text_rect):
        ...
        profiler.start_timer('output_tables')
    ...
    def get_page_output(doc, pno: int, margins, textflags, FILENAME, IGNORE_IMAGES, IGNORE_GRAPHICS):
        """Process one page (page number: pno), offloading all Markdown text formatting to C."""
        profiler.start_timer('get_page_output')

        page = doc[pno]  # Access pno to fix "not accessed" error
        page.remove_rotation()  # ensure rotation=0
        parms = Parameters()
        parms.page = page
        ...
        parms.written_images = []  # stores already written images
        parms.accept_invisible = page_is_ocr(page) or ignore_alpha
        # Always initialize table rects, even if no tables detected
        parms.tab_rects0 = []
        parms.tab_rects = {}
        ...
        if not IGNORE_IMAGES:
            ...
            # filter small / out-of-clip images
            img_info = [
                i for i in img_info
            ...
            parms.images = img_info[:30]  # keep largest 30
        ...
        # locate tables if they match a basic strategy
        ...
        # graphics paths (vector graphics)
        profiler.start_timer('process_graphics')
        ...
        profiler.end_timer('process_graphics')

        # Initialize empty markdown string for per-page processing
        # The full document markdown is handled at the document level
        parms.md_string = ""

        # append tables and images using Python logic
        profiler.start_timer('output_tables_images')
        tab_rects_only = [rect for i, rect in parms.tab_rects0]
        for rect in column_boxes(parms.page, paths=paths, no_image_text=not force_text,
                                textpage=None, avoid=tab_rects_only + parms.vg_clusters0,
                                footer_margin=margins[3], header_margin=margins[1],
                                ignore_images=IGNORE_IMAGES):
            parms.md_string += output_tables(parms, rect)
            parms.md_string += output_images(parms, rect, force_text)

        # finalize any remaining tables/images
        parms.md_string += output_tables(parms, None)
        parms.md_string += output_images(parms, None, force_text)
        profiler.end_timer('output_tables_images')
        ...
        profiler.end_timer('get_page_output')
        return parms
    ...
    profiler.start_timer('c_markdown_generation')
    full_document_md = c_to_markdown(FILENAME) if pages else ""
    profiler.end_timer('c_markdown_generation')
    
    profiler.start_timer('page_processing_loop')
    for pno in pages:
        profiler.start_timer(f'page_{pno}_processing')
        ...
        if page_chunks is False:
            ...
            # For page chunks, we need to extract the relevant portion of the full markdown
            # For now, we'll use the page-specific tables/images only
            # TODO: This needs proper page splitting logic for the C markdown
            ...
            document_output.append(
                {
                    ...
                    "text": parms.md_string,  # This now contains only tables/images
                    "words": parms.words,
                }
            )
        profiler.end_timer(f'page_{pno}_processing')
        del parms
    profiler.end_timer('page_processing_loop')
