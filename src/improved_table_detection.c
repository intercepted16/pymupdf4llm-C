...
static float analyze_table_content(fz_context *ctx, fz_stext_page *textpage, fz_rect table_bbox)
{
    ...
    for (fz_stext_block *block = textpage->first_block; block; block = block->next)
    {
        ...
        if (!fz_rect_contains_custom(table_bbox, block->bbox))
            continue;
                                 ...
                                 fz_context *ctx, fz_stext_page *textpage)
{
    ...
    for (int i = 0; i < block_count && candidate_count < max_candidates; i++)
    {
        ...
        for (int j = 0; j < nearby_count && region_count < 100; j++)
        {
            ...
            if (processed[block_idx] || block_idx == i)
                continue;
...
extern int page_has_table(const char *pdf_path, int page_number)
{
    ...
    fz_try(ctx)
    {
        ...
        if (!blocks)
            fz_throw(ctx, FZ_ERROR_GENERIC, "Cannot allocate memory");
        int block_count = 0;
    ...
    fz_try(ctx)
    {
        ...
        for (int i = 0; i < block_count && !has_table; i++)
        {
            ...
            for (int j = 0; j < block_count; j++)
            {
                if (i == j)
                    continue;
                if (fabs(blocks[i].y0 - blocks[j].y0) < EPSILON)
                    row_count++;
                if (fabs(blocks[i].x0 - blocks[j].x0) < EPSILON)
                    col_count++;
            }
